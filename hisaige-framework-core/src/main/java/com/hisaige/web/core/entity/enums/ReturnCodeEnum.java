package com.hisaige.web.core.entity.enums;
...
public enum ReturnCodeEnum {
    ...
    //用户配置
    LOGOUT_FAILED(-11000, "user logout failed", "用户登出失败"),
    LOGIN_FAILED(-11001, "invalid userName or password", "用户名或密码错误"),
    USER_NOT_EXIST(-11002, "user not exist", "用户信息不存在"),
    ROLE_NOT_EXIST(-11003, "role not exist", "角色信息不存在"),
    MENU_NOT_EXIST(-11004, "menu not exist", "菜单信息不存在"),
    TOKEN_EXPIRED(-11005, "token expired", "token已过期"),
    AUTHENTICATION_FAILED(-11006, "authentication failed", "鉴权失败"),
    TOKEN_EMPTY(-11007, "token is empty!", "令牌不能为空"),
    ;
